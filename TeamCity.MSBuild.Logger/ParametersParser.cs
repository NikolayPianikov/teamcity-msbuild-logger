...
internal class ParametersParser : IParametersParser
{
    ...
    private static bool ApplyParameter(Parameters parameters, string parameterName, string? parameterValue, out string? error)
    {
        if (parameters == null) throw new ArgumentNullException(nameof(parameters));
        ...
        switch (parameterNameUpper)
        {
            ...
            case "VERBOSITY":
                ...
                if (parameterValue is not null && TryApplyVerbosityParameter(parameterValue, out error, out var verbosity))
                {
