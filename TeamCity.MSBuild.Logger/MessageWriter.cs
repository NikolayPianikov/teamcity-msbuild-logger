...
internal class MessageWriter: IMessageWriter
{
    ...
    private void WritePretty(string formattedString)
    {
        if (formattedString == null) throw new ArgumentNullException(nameof(formattedString));
        WritePretty(_context.IsVerbosityAtLeast(LoggerVerbosity.Normal) ? _context.CurrentIndentLevel : 0, formattedString);
    }

    private void WritePretty(int indentLevel, string formattedString)
    {
        if (formattedString == null) throw new ArgumentNullException(nameof(formattedString));
    ...
    private void PrintTargetNamePerMessage(BuildEventArgs e, bool lightenText)
    {
        ...
        var ctx = _context.LastDisplayedBuildEventContext == default ? default : _context.LastDisplayedBuildEventContext;
        var flag2 = ctx is not null && ComparerContextNodeIdTargetId.Shared.Equals(buildEventContext, ctx);
        TargetStartedEventMinimumFields? eventMinimumFields = default;
        if (!flag2)
        {
        ...
        if (!flag1 || eventMinimumFields is null)
        {
            return;
        }
