...
internal class TeamCityHierarchicalMessageWriter : IHierarchicalMessageWriter, ILogWriter, IDisposable
{
    ...
    private void Write(string? message, Flow flow)
    {
    ...
    private class Flow: IDisposable
    {
        ...
        private readonly Stack<ITeamCityWriter> _blocks = new();
        private readonly IEventContext _eventContext;
        private readonly bool _isMainFlow;

        public Flow(ITeamCityWriter writer, IEventContext eventContext, bool isMainFlow)
        {
        ...
        public void Write(IServiceMessage message)
        {
            if (message == null) throw new ArgumentNullException(nameof(message));
